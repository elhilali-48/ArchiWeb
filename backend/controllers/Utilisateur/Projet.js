      ...
      if (projet) {
        ...
        if (!resultatsEtudiant) {
          ...
          res
            ...
            .json({
              status: res.statusCode,
              message: "Vous etes bien inscrits dans ce projet",
            });
        } else {
      ...
      if (projet) {
        ...
        if (resultatsEtudiant) {
          ...
          if (competence) {
            ...
            await projet.save();
            res.status(200).json({ status: res.statusCode, message: "Le statut de la compétence a été modifié avec succès" });
          } else {
            res.status(401).json({ status: res.statusCode, error: "Aucune compétence trouvée avec cet ID" });
          }
        } else {
